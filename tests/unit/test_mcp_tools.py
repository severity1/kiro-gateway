...
class TestCallKiroMCPAPI:
    ...
    @pytest.mark.asyncio
    async def test_mcp_api_includes_profile_arn(self, mock_auth_manager):
        """
        What it does: Verifies the posted MCP request body includes profileArn.
        Purpose: Guard against the omission that caused a 400 'profileArn is required'.
        """
        print("Setup: Mocking MCP API response to capture posted body...")
        query = "Python tutorials"
        ...
        print("Inspecting posted request body...")
        assert mock_post.call_count == 1
        posted_body = mock_post.call_args.kwargs["json"]

        print(f"Comparing profileArn: Got '{posted_body.get('profileArn')}'")
        assert "profileArn" in posted_body
        assert posted_body["profileArn"] == mock_auth_manager.profile_arn
